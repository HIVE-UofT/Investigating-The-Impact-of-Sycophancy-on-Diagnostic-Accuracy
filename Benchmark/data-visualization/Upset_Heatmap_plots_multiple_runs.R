...
df <- map_dfr(input_files, process_csv, .id = "run_number")
...
upset_correct_df <- df %>%
  filter(case_condition_cat == "baseline") %>%
  select(case_origin_cat, case_id_str, model, acc_pass1, run_number) %>%
  mutate(acc_pass1 = tidyr::replace_na(acc_pass1, FALSE)) %>%
  pivot_wider(
    id_cols = c(run_number, case_origin_cat, case_id_str),
    names_from = model,
...
model_family_lookup <- c(
  ...
  "Gemini-3.5" = "Proprietary general purpose",
